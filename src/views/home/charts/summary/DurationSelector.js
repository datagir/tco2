...
export default function Distance() {
  const { possessionDuration, setPossessionDuration } =
    ...
    <Wrapper>
      <>
        <Button
          ...
          onClick={() =>
            setPossessionDuration((possessionDuration) =>
              possessionDuration - 1 < 1 ? 1 : possessionDuration - 1
            )
          }
        >
          <svg
        ...
        <Range
          ...
          min={1}
          max={20}
          values={[possessionDuration]}
          onChange={(values) => setPossessionDuration(values[0])}
          renderTrack={({ props, children }) => (
            <Track {...props}>{children}</Track>
          )}
          renderThumb={({ props }) => (
            <Thumb {...props} aria-label='Durée de possession'>
              TCO sur {possessionDuration} an{possessionDuration > 1 ? 's' : ''}
            </Thumb>
          )}
        />
        <Button
          aria-label='plus'
          onClick={() =>
            setPossessionDuration((prevPosition) =>
